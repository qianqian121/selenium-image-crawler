        ...
        try:
            ...
            while True:
                doc_height_new = driver.execute_script(
                    "window.scrollTo(0, document.body.scrollHeight);var lenOfPage=document.body.scrollHeight;return lenOfPage;")
                button_load_rest = driver.find_element_by_xpath("/html/body/div[6]/div[1]/div/div[3]/a")
                if button_load_rest is None:
                    break
                if button_load_rest.is_displayed() != True:
                     time.sleep(2)
                     print("waiting 2 seconds for clickable button...")
                     counter1 +=1
                     if counter1 > 10:
                         print("page load comopleted, end of page")
                         break;
                     continue

                print(doc_height_new)
                filename = "output" + str(counter) + ".html"
                target = open(filename, 'w')
                target.write(str(driver.page_source.encode('ascii', 'ignore')))
                target.close()
